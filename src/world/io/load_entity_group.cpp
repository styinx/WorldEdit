...
void read_blocks_quads(const assets::config::node& node, entity_group& group_out)
{
   for (const auto& key_node : node) {
      if (not string::iequals(key_node.key, "Quad")) continue;

      block_description_quad quad;

      for (const auto& prop : key_node) {
         if (string::iequals(prop.key, "Vertex0")) {
            quad.vertices[0] = {prop.values.get<float>(0), prop.values.get<float>(1),
                                prop.values.get<float>(2)};
         }
         else if (string::iequals(prop.key, "Vertex1")) {
            quad.vertices[1] = {prop.values.get<float>(0), prop.values.get<float>(1),
                                prop.values.get<float>(2)};
         }
         else if (string::iequals(prop.key, "Vertex2")) {
            quad.vertices[2] = {prop.values.get<float>(0), prop.values.get<float>(1),
                                prop.values.get<float>(2)};
         }
         else if (string::iequals(prop.key, "Vertex3")) {
      ...
      group_out.blocks.quads.push_back(quad);
   }
}
...
void read_blocks_custom(const assets::config::node& node, entity_group& group_out)
{
   for (const auto& key_node : node) {
      ...
      if (string::iequals(key_node.key, "Stairway")) {
         block.mesh_description = block_custom_mesh_description_stairway{};
      ...
      else if (string::iequals(key_node.key, "StairwayFloating")) {
         block.mesh_description = block_custom_mesh_description_stairway_floating{};
         block_custom_mesh_description_stairway_floating& stairway =
      ...
      else if (string::iequals(key_node.key, "Ring")) {
         block.mesh_description = block_custom_mesh_description_ring{};
         block_custom_mesh_description_ring& ring = block.mesh_description.ring;
         ...
         for (const auto& prop : key_node) {
            ...
            else if (string::iequals(prop.key, "InnerRadius")) {
               ring.inner_radius = prop.values.get<float>(0);
            }
            else if (string::iequals(prop.key, "OuterRadius")) {
               ring.outer_radius = prop.values.get<float>(0);
            }
            else if (string::iequals(prop.key, "Height")) {
               ring.height = prop.values.get<float>(0);
            }
            else if (string::iequals(prop.key, "Segments")) {
               ring.segments = prop.values.get<uint16>(0);
            }
            else if (string::iequals(prop.key, "FlatShading")) {
               ring.flat_shading = true;
            }
            else if (string::iequals(prop.key, "TextureLoops")) {
               ring.texture_loops = prop.values.get<float>(0);
            }
            else if (string::iequals(prop.key, "SurfaceMaterials")) {
               for (uint32 i = 0; i < block.surface_materials.size(); ++i) {
                  block.surface_materials[i] = prop.values.get<uint8>(i);
      ...
      else if (string::iequals(key_node.key, "CubicCurve")) {
         block.mesh_description = block_custom_mesh_description_curve{};
         block_custom_mesh_description_curve& curve = block.mesh_description.curve;
         ...
         for (const auto& prop : key_node) {
            ...
            else if (string::iequals(prop.key, "Width")) {
               curve.width = prop.values.get<float>(0);
            }
            else if (string::iequals(prop.key, "Height")) {
               curve.height = prop.values.get<float>(0);
            }
            else if (string::iequals(prop.key, "Segments")) {
               curve.segments = prop.values.get<uint16>(0);
            }
            else if (string::iequals(prop.key, "TextureLoops")) {
               curve.texture_loops = prop.values.get<float>(0);
            }
            else if (string::iequals(prop.key, "P0")) {
               curve.p0 = {prop.values.get<float>(0), prop.values.get<float>(1),
                           prop.values.get<float>(2)};
            }
            else if (string::iequals(prop.key, "P1")) {
               curve.p1 = {prop.values.get<float>(0), prop.values.get<float>(1),
                           prop.values.get<float>(2)};
            }
            else if (string::iequals(prop.key, "P2")) {
               curve.p2 = {prop.values.get<float>(0), prop.values.get<float>(1),
                           prop.values.get<float>(2)};
            }
         ...
         for (const auto& prop : key_node) {
            if (string::iequals(prop.key, "Rotation")) {
               ...
               cone.segments = prop.values.get<uint16>(0);
            }
            else if (string::iequals(prop.key, "FlatShading")) {
               cone.flat_shading = true;
            }
            else if (string::iequals(prop.key, "SurfaceMaterials")) {
               for (uint32 i = 0; i < block.surface_materials.size(); ++i) {
                  block.surface_materials[i] = prop.values.get<uint8>(i);
      ...
      else if (string::iequals(key_node.key, "Arch")) {
         block.mesh_description = block_custom_mesh_description_arch{};
         block_custom_mesh_description_arch& arch = block.mesh_description.arch;
         ...
         for (const auto& prop : key_node) {
            if (string::iequals(prop.key, "Rotation")) {
               ...
               arch.size = {prop.values.get<float>(0), prop.values.get<float>(1),
                            prop.values.get<float>(2)};
            }
            else if (string::iequals(prop.key, "CrownLength")) {
               arch.crown_length = prop.values.get<float>(0);
            }
            else if (string::iequals(prop.key, "CrownHeight")) {
               arch.crown_height = prop.values.get<float>(0);
            }
            else if (string::iequals(prop.key, "CurveHeight")) {
               arch.curve_height = prop.values.get<float>(0);
            }
            else if (string::iequals(prop.key, "SpanLength")) {
               arch.span_length = prop.values.get<float>(0);
            }
            else if (string::iequals(prop.key, "Segments")) {
               arch.segments = prop.values.get<uint16>(0);
            }
            else if (string::iequals(prop.key, "SurfaceMaterials")) {
               for (uint32 i = 0; i < block.surface_materials.size(); ++i) {
                  block.surface_materials[i] = prop.values.get<uint8>(i);
...
void read_blocks_hemispheres(const assets::config::node& node, entity_group& group_out)
{
   for (const auto& key_node : node) {
      if (not string::iequals(key_node.key, "Hemisphere")) continue;
      ...
      group_out.blocks.hemispheres.push_back(hemisphere);
   }
}

void read_blocks_pyramids(const assets::config::node& node, entity_group& group_out)
{
   for (const auto& key_node : node) {
      if (not string::iequals(key_node.key, "Pyramid")) continue;
      ...
      group_out.blocks.pyramids.push_back(pyramid);
   }
}
...
void clamp_block_material_indices(entity_group& group_out, output_stream& output) noexcept
{
   ...
   for (uint32 block_index = 0;
        block_index < group_out.blocks.hemispheres.size(); ++block_index) {
      block_description_hemisphere& hemisphere =
         group_out.blocks.hemispheres[block_index];

      for (uint8& material : hemisphere.surface_materials) {
         if (material >= max_material) {
            output.write(
               "Warning! Hemisphere block '{}' has out of range material "
               "index. Setting to zero.\n",
               block_index);
   ...
   for (uint32 block_index = 0; block_index < group_out.blocks.pyramids.size();
        ++block_index) {
      block_description_pyramid& pyramid = group_out.blocks.pyramids[block_index];

      for (uint8& material : pyramid.surface_materials) {
         if (material >= max_material) {
            output
               .write("Warning! Pyramid block '{}' has out of range material "
                      "index. Setting to zero.\n",
                      block_index);
                                   ...
                                   output_stream& output) -> entity_group
{
   try {
      ...
      for (auto& key_node : assets::config::read_config(entity_group_data)) {
         ...
         else if (string::iequals(key_node.key, "BlocksQuads"sv)) {
         ...
         else if (string::iequals(key_node.key, "BlocksHemispheres"sv)) {
            group.blocks.hemispheres.reserve(key_node.values.get<uint32>(0));

            read_blocks_hemispheres(key_node, group);
         }
         else if (string::iequals(key_node.key, "BlocksPyramids"sv)) {
