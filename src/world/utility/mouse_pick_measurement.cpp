   ...
   -> std::optional<measurement_id>
{
   for (const measurement& measurement : measurements) {
      ...
      const measurement_metrics metrics = get_measurement_metrics(measurement);

      const float4 centrePS = view_projection_matrix * float4{metrics.centre, 1.0f};
      ...
      const char* const text_end =
         fmt::format_to_n(text.data(), text.size(), "{:.2f}m", metrics.length).out;
